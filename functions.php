...
require_once get_template_directory() . '/inc/wallet.php'; // digitalni wallet (Google) — koristi paketi.php
require_once get_template_directory() . '/inc/wallet-apple.php'; // Apple Wallet (.pkpass)
...
function dry65_scripts() {
    wp_enqueue_style('dry65-style', get_stylesheet_uri(), [], '1.1.10');
...
function dry65_jobposting_schema() {
    ...
    foreach ($positions as $pos) {
        $job = [
            ...
            '@type'    => 'JobPosting',
            'title'    => $pos['title'],
            'description' => function_exists('t') ? t($pos['desc']) : $pos['desc'],
            'datePosted'   => $today,
...
add_action('wp_head', function() {
    ...
    $howto = [
        ...
        'description'   => t('Jednostavan vodič kako da posetite Dry65 walk-in blowout hair bar u Novom Beogradu.'),
        'totalTime'     => 'PT30M',
        'estimatedCost' => [
            '@type'         => 'MonetaryAmount',
            'currency'      => 'RSD',
